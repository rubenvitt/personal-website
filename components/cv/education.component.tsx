    ...
    return <div className="relative pt-16 pb-20 px-4 sm:px-6 lg:pt-24 lg:pb-28 lg:px-8">
        <div className="relative max-w-7xl mx-auto">
            ...
            <div className="bg-white shadow overflow-hidden sm:rounded-md mt-3 lg:mt-6">
                {
                    studies.map((elem, i) => {
                        return <EducationItem study={elem} i={i}/>
                    })
                }
            </div>
        </div>
                ...
                <div className="px-4 py-4 sm:px-6">
                    <div className="flex items-center justify-between">
                        <div className="text-sm leading-5 font-medium text-orange-600 truncate">
                            {study.subject} <span className={study.status === Status.COMPLETED ? 'text-gray-500' : 'text-gray-300'}>| {study.degree}</span>
                        </div>
                        <div className="ml-2 flex-shrink-0 flex">
              <span
                  className={"px-2 inline-flex text-xs leading-5 font-semibold rounded-full " + (
                      study.status === Status.INCOMPLETE ? 'bg-red-100 text-red-800' :
                          study.status === Status.SWITCHED ? 'bg-yellow-100 text-yellow-800'
                ...
                {study.duration.start.toLocaleDateString()} until {study.duration.end?.toLocaleDateString() ?? 'now'}
              </span>
                        </div>
                    </div>
                </div>
