...
export const SlideListElement = ({skill}: SlideListElementProps) => {
    return <li key={skill.title} className="col-span-1 bg-white rounded-lg shadow hover:bg-gray-100">
                    ...
                    <div className="-mt-px">
                        {
                            skill.certificates.slice(0,3).map(cert => {
                                return <div key={cert.url} className="flex-1 border-b flex border-r border-gray-200">
                                    <a href={cert.url} target='_blank'
                                       className="relative -mr-px w-0 flex-1 inline-flex items-center justify-center py-2 text-xs leading-5 text-gray-700 font-medium border border-transparent rounded-bl-lg hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 transition ease-in-out duration-150">
